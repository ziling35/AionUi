...
[Why Choose LingAI?](#-why-choose-lingai-over-claude-cowork) ·
...
**LingAI is more than a chat client.** It's a Cowork platform where AI agents work alongside you on your computer — reading files, writing code, browsing the web, and automating tasks. You see everything the agent does, and you're always in control.

|                                 | Traditional AI Chat Clients | **LingAI (Cowork)**                                                                                                     |
| :------------------------------ | :-------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
...
LingAI ships with a complete AI agent engine. Unlike tools that require you to install CLI agents separately, **LingAI works the moment you install it**.
...
If you already use Claude Code, Codex, Hermes Agent, or OpenClaw, LingAI auto-detects them and lets you Cowork with all of them — alongside the built-in agent.

**Supported Agents:** Built-in Agent (zero setup) • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • AI CLI (aionrs, the Rust-based backend service shipped with LingAI) • Snow CLI • Hermes Agent • Cursor Agent and more
...
- **Parallel multi-agent execution** — Leader breaks tasks into subtasks and delegates to Teammate agents running in parallel; each Teammate uses its own model via ACP (Agent Communication Protocol, LingAI's multi-agent coordination layer), Gemini, or Aionrs
- **Leader orchestration** — Leader assigns, tracks, and aggregates results; supported backends include Claude Code, Codex, Hermes Agent, Gemini, Snow CLI, and AI CLI
...
- **Supported backends** — Claude Code, Codex, Gemini, Snow CLI, AI CLI (aionrs); other ACP backends with `mcpCapabilities.stdio` are auto-supported
...
Other AI apps give you a chatbox with your API key. **LingAI gives you a full Cowork agent.**
...
| AWS Bedrock credentials    | Bedrock-powered Agent via AI CLI (aionrs) |
| Ollama / LM Studio (local) | Local model Cowork Agent                    |
| NewAPI Gateway             | Unified access to 20+ models                |

Same agent capabilities — file read/write, web search, image generation, tool use — regardless of which model powers it. LingAI supports **30+ AI platforms** including cloud services and local deployments.
...
LingAI also supports [NewAPI](https://github.com/QuantumNous/new-api) gateway service — a unified AI model hub that aggregates and distributes various LLMs. Flexibly switch between different models in the same interface to meet various task requirements.
...
- **Three-tier Skills** — Builtin skills (shipped with LingAI), custom skills (your own), and Extension skills (contributed by third-party extensions); enable/disable per conversation with the skill indicator
...
LingAI supports three skill layers: **built-in** skills (shipped with the app), **custom** skills (user-defined), and **extension** skills (loaded from the Extension SDK).
...
LingAI includes **21 professional assistants** with predefined capabilities, extendable through custom skills:
...
**Custom Skills**: Create skills in the `skills/` directory, enable/disable skills for assistants to extend AI capabilities. Skills come from three sources: builtin (shipped with LingAI), custom (your own), and Extension (contributed via the Extension SDK). Built-in skills include `pptx`, `docx`, `pdf`, `xlsx`, `mermaid`, and more.
...
_Your 24/7 AI assistant — access LingAI from any device, anywhere._
...
> **Setup:** LingAI Settings → WebUI Settings → Channel, configure the Bot Token.
...
> [Remote Internet Access Tutorial](https://github.com/iOfficeAI/LingAI/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **Keep-awake** — LingAI automatically prevents system sleep while tasks are active, and detects missed triggers after wake
...
> [Image generation model configuration guide](https://github.com/iOfficeAI/LingAI/wiki/LingAI-Image-Generation-Tool-Model-Configuration-Guide)
...
## 🤔 Why Choose LingAI Over Claude Cowork?
...
LingAI is a **free and open-source Multi-AI Agent Desktop**. Compared to Claude Cowork which only runs on macOS and is locked to Claude, LingAI is its full-model, cross-platform enhanced version.

| Dimension     | Claude Cowork | LingAI                                                    |
| :------------ | :------------ | :-------------------------------------------------------- |
...
A: <strong>No.</strong> LingAI has a built-in AI agent that works immediately after installation. Just enter any API key to get started. If you also have CLI tools like Claude Code or Gemini CLI installed, LingAI will auto-detect and integrate them for even more capabilities.
</details>

<details>
<summary><strong>Q: What can I do with LingAI?</strong></summary>
A: LingAI is your <strong>private Cowork workspace</strong>. The built-in agent can batch organize folders, process Excel data, generate documents, search the web, and generate images. With Multi-Agent Mode, you can also leverage Claude Code, Codex, and other powerful CLI agents through the same interface.
</details>

<details>
<summary><strong>Q: Is it free?</strong></summary>
A: LingAI is completely free and open source. You only pay for the API usage of whichever provider you choose, and you can use API keys from any provider you prefer.
</details>

<details>
<summary><strong>Q: Can I run LingAI on a server (headless)?</strong></summary>
A: Yes — LingAI WebUI mode runs as a standalone HTTP server. See the WebUI section above for setup instructions.
</details>
...
> **Made a video about LingAI?** [Let us know on X](https://x.com/LingAI) and we'll feature it here!
...
- [📖 Complete Installation Guide](https://github.com/iOfficeAI/LingAI/wiki/Getting-Started) — Detailed steps from download to configuration
- [⚙️ LLM Configuration Guide](https://github.com/iOfficeAI/LingAI/wiki/LLM-Configuration) — Multi-platform AI model configuration
- [🤖 Multi-Agent Mode Setup](https://github.com/iOfficeAI/LingAI/wiki/ACP-Setup) — Integrate terminal AI agents
- [🔌 MCP Tool Configuration](https://github.com/iOfficeAI/LingAI/wiki/MCP-Configuration-Guide) — Model Context Protocol server setup
- [🌐 WebUI Configuration Guide](https://github.com/iOfficeAI/LingAI/wiki/WebUI-Configuration-Guide) — Complete WebUI setup and configuration tutorial

**🎯 Use Cases**

- [📁 File Management](https://github.com/iOfficeAI/LingAI/wiki/file-management) — Smart file organization
- [📊 Excel Processing](https://github.com/iOfficeAI/LingAI/wiki/excel-processing) — AI-driven data processing
- [🎨 Image Generation](https://github.com/iOfficeAI/LingAI/wiki/LingAI-Image-Generation-Tool-Model-Configuration-Guide) — AI image creation
- [📚 More Use Cases](https://github.com/iOfficeAI/LingAI/wiki/Use-Cases-Overview)

**❓ Support & Help**

- [❓ FAQ](https://github.com/iOfficeAI/LingAI/wiki/FAQ) — Questions and troubleshooting
- [🔧 Configuration & Usage Tutorials](https://github.com/iOfficeAI/LingAI/wiki/Configuration-Guides) — Complete configuration documentation
...
- [GitHub Discussions](https://github.com/iOfficeAI/LingAI/discussions) — share ideas and exchange tips
- [Report Issues](https://github.com/iOfficeAI/LingAI/issues) — bugs and feature requests
- [Release Updates](https://github.com/iOfficeAI/LingAI/releases) — get the latest version
...
LingAI local development uses two repositories: LingAI for the Electron frontend and AionCore for the local backend. See the [Development Guide](docs/contributing/development.md) for the full macOS, Linux, and Windows setup.
...
[Report Bug](https://github.com/iOfficeAI/LingAI/issues) · [Request Feature](https://github.com/iOfficeAI/LingAI/issues)
